"use client";

import { Component, ReactNode, useContext } from "react";
import { AlertTriangle, RefreshCcw, Home } from "lucide-react";
import Link from "next/link";
import { ThemeContext } from "@/contexts/ThemeContext";

interface Props {
  children: ReactNode;
  fallback?: ReactNode;
}

interface State {
  hasError: boolean;
  error?: Error;
}

export class ErrorBoundary extends Component<Props, State> {
  constructor(props: Props) {
    super(props);
    this.state = { hasError: false };
  }

  static getDerivedStateFromError(error: Error): State {
    return { hasError: true, error };
  }

  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    console.error("ErrorBoundary caught an error:", error, errorInfo);
  }

  handleRetry = () => {
    this.setState({ hasError: false, error: undefined });
  };

  render() {
    if (this.state.hasError) {
      if (this.props.fallback) {
        return this.props.fallback;
      }

      return <ErrorFallback error={this.state.error} onRetry={this.handleRetry} />;
    }

    return this.props.children;
  }
}

// Function component fallback that can access ThemeContext
function ErrorFallback({ error, onRetry }: { error?: Error; onRetry: () => void }) {
  const themeContext = useContext(ThemeContext);
  const isDark = themeContext?.theme === "dark";

  const pageBg = isDark ? "#1A1A1A" : "#F5F5F5";
  const cardBg = isDark ? "#2D2D2D" : "#FFFFFF";
  const textColor = isDark ? "#FFFFFF" : "#1A1A1A";
  const textMuted = isDark ? "rgba(255,255,255,0.6)" : "rgba(0,0,0,0.6)";
  const primaryColor = "#00568C";

  return (
    <div
      className="min-h-screen flex items-center justify-center p-4"
      style={{ backgroundColor: pageBg }}
    >
      <div
        className="max-w-md w-full rounded-2xl border-2 p-6 text-center"
        style={{
          backgroundColor: cardBg,
          borderColor: primaryColor,
          boxShadow: `4px 4px 0px ${primaryColor}`,
        }}
      >
        {/* Icon */}
        <div
          className="w-16 h-16 mx-auto mb-4 rounded-full flex items-center justify-center"
          style={{ backgroundColor: isDark ? "#5C3A3A" : "#FFE4C9" }}
        >
          <AlertTriangle className="w-8 h-8 text-[#FF6B6B]" />
        </div>

        {/* Title */}
        <h2 className="font-felipa text-2xl mb-2" style={{ color: textColor }}>
          อุ๊ปส์! มีบางอย่างผิดพลาด
        </h2>

        {/* Description */}
        <p className="font-kanit text-sm mb-6" style={{ color: textMuted }}>
          เกิดข้อผิดพลาดที่ไม่คาดคิด กรุณาลองใหม่อีกครั้ง
        </p>

        {/* Error Details (Development only) */}
        {process.env.NODE_ENV === "development" && error && (
          <div
            className="mb-6 p-3 rounded-lg text-left overflow-auto max-h-32"
            style={{ backgroundColor: isDark ? "#3D3D3D" : "#F0F0F0" }}
          >
            <code className="text-xs text-red-500 font-mono">
              {error.message}
            </code>
          </div>
        )}

        {/* Actions */}
        <div className="flex gap-3 justify-center">
          <button
            onClick={onRetry}
            className="flex items-center gap-2 px-4 py-2 rounded-xl border-2 font-kanit text-sm transition-all hover:scale-105"
            style={{
              borderColor: primaryColor,
              backgroundColor: isDark ? "#1A3A4D" : "#C5E8FF",
              color: primaryColor,
            }}
          >
            <RefreshCcw className="w-4 h-4" />
            ลองใหม่
          </button>
          <Link
            href="/"
            className="flex items-center gap-2 px-4 py-2 rounded-xl border-2 font-kanit text-sm transition-all hover:scale-105"
            style={{
              borderColor: primaryColor,
              backgroundColor: primaryColor,
              color: "#FFFFFF",
            }}
          >
            <Home className="w-4 h-4" />
            หน้าแรก
          </Link>
        </div>
      </div>
    </div>
  );
}

// Hook-based error boundary wrapper for functional components
export function withErrorBoundary<P extends object>(
  WrappedComponent: React.ComponentType<P>,
  fallback?: ReactNode
) {
  return function WithErrorBoundaryWrapper(props: P) {
    return (
      <ErrorBoundary fallback={fallback}>
        <WrappedComponent {...props} />
      </ErrorBoundary>
    );
  };
}
